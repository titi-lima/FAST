...
def extract_test_functions(go_src: str) -> list[str]:
    # Match `func TestXxx(t *testing.T) { ... }` and extract bodies
    text = strip_comments(go_src)
    tests: list[str] = []
    # Rough parsing: find "func Test...(" then capture balanced braces
...
def main() -> None:
    ...
    ap.add_argument(
        "--tests-dir", required=True, help="Path to directory with *_test.go"
    )
    ap.add_argument(
        "--out", required=True, help="Output file path, e.g., .fast/in/gotest-bbox.txt"
    )
    args = ap.parse_args()
