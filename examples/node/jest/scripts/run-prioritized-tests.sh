...
# End-to-end: install deps, generate bbox, prioritize, map, then run Jest in order

ROOT_DIR="$(cd "$(dirname "$0")"/../.. && pwd)"   # examples/node
EX_DIR="$ROOT_DIR/jest"                               # this example root
...
echo "[5/6] Map prioritized IDs to Jest (file + name)"
"$PY" "$EX_DIR/scripts/build-prioritized-tests.py" \
  --out-dir "$OUT_DIR" \
  --tests "$DATA_DIR/test-names.tsv" \
  --out "$DATA_DIR/prioritized-tests.tsv"

echo "[6/6] Run Jest tests in prioritized order"
while IFS=$'\t' read -r file name; do
  [ -z "$file" ] && continue
  # Escape regex for Jest's -t/--testNamePattern and anchor it
  PATTERN=$("$PY" -c 'import re,sys; print(re.escape(sys.argv[1]))' "$name")
  echo "==> $name ($file)"
  (cd "$EX_DIR" && npx --yes jest "$file" -t "^$PATTERN$" --runInBand --silent) | cat || true
